...
@options = {
    :digest_auth => {:username => ENV['JENKINS_USERNAME'], :password => ENV['JENKINS_PASSWORD'] }, :verify => false
}

# the key of this mapping must be a unique identifier for your job, the according value must be the name that is specified in jenkins , :pre_job => 'PRE_BUILD'
job_mapping = {
  'JOB' => { :job => 'master'}
}
...
def get_completion_percentage(job_name)
  ...
  last_duration = (prev_build_info["estimatedDuration"] / 1000).round(2)
...
def get_json_for_job(job_name, build = 'lastBuild')
  ...
  response = HTTParty.get("#{ENV['JENKINS_URI']}/job/#{job_name}/#{build}/api/json", @options)
